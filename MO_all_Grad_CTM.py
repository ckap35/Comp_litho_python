...
for num_iterations in range(30000):
    ...
    m_iter=m_iter+0.01*direction_mask;        # Flip pixel sign at position of max gradient
    ...
    if (error_post>=error_pre):
         repeats+=1;
         print('Grad step failed to improve cost')
         for p in range(1,5):
            i=random.randint(1,m.shape[0]-1)     #set pixel of interest to pixel with largest gradient value
            j=random.randint(1,m.shape[0]-1) 
            m_iter[i,j]=m_iter[i,j]+5*random.uniform(0, 1)*direction_mask[i,j];  
